import{A,i as ue,a as me}from"./assets/favorites-B296tOCn.js";import{a as T,i as C}from"./assets/vendor-BT7OT44q.js";class ve{constructor(){this.baseURL="https://your-energy.b.goit.study/api/",this.page=0}async getExercises(t,s,n){try{return(await T.get(`${this.baseURL}exercises?${t}=${s}&page=${n}&limit=10`)).data}catch(a){console.log(a)}}async getExercisesById(t){try{return(await T.get(`${this.baseURL}exercises/${t}`)).data}catch(s){console.log(s)}}async getFilter(t,s){try{return(await T.get(`${this.baseURL}filters?filter=${t}&page=${s}&limit=12`)).data}catch(n){console.log(n)}}async patchRating(t,s){try{return this.page+=1,(await T.patch(`${this.baseURL}exercises/${t}/rating`,s)).data}catch(n){console.log(n)}}}const fe=new A,L=document.querySelector(".js-list"),R=document.getElementById("pagination-numbers");let f=1;P("Muscles",f);async function P(e,t){try{const{results:s,totalPages:n}=await fe.getFilter(e,t);_e({results:s,totalPages:n}),pe(s)}catch(s){console.log(s)}}function pe(e){if(!L)return;if(L.innerHTML="",e.length===0){L.insertAdjacentHTML("beforeend",'<p class="filters__empty">No exercises found</p>');return}const t=e.map(({filter:s,name:n,imgURL:a})=>`
  <li class="filters__item">
    ...
    `);else{localFavorite.innerHTML="";const t=e.map(({_id:s,name:n,burnedCalories:a,bodyPart:c,target:i})=>`
      ...
      </li>`).join("");localFavorite.insertAdjacentHTML("beforeend",t)}}function xe(){document.querySelector(".js-list").addEventListener("click",ye)}document.addEventListener("load",xe);const Le=new ve;let $=!1,X;const N=document.querySelector(".modal-exercises"),g=document.querySelector(".overlay"),O=document.querySelector(".js-list");O==null||O.addEventListener("click",Ee);async function Ee(e){if(e.target.closest(".card__btn"))try{const t=e.target.closest(".card__btn").getAttribute("data-id"),s=await Le.getExercisesById(t);X=t;const n=ke(s);Se(n),$e(),document.querySelector(".modal-exercises__btn-favorites").addEventListener("click",()=>Fe(s)),document.querySelector(".modal-exercises__btn-close").addEventListener("click",G)}catch(t){console.log(t)}}function $e(){const e=window.innerWidth-document.body.offsetWidth+"px";N.classList.remove("hidden"),g.classList.remove("hidden"),document.body.style.paddingRight=e,document.body.style.overflow="hidden"}function Se(e){N.innerHTML=e,Te()}function we(e){const t="#EEA10C",s="#F4F4F4";let a="";for(let r=0;r<5;r++){const o=`starGradient${r}`,d=r+1<=e?100:r<e?e%1*100:0,p=[{offset:"0%",color:t,opacity:"1"},{offset:`${d}%`,color:t,opacity:"1"},{offset:`${d+1}%`,color:s,opacity:"0.20"}],j=`
...
`}function Te(){const e=JSON.parse(localStorage.getItem("exerciseData")),t=document.querySelector(".modal-exercises__btn-favorites");e!=null&&e.some(s=>s._id===X)?($=!0,t.innerHTML=ee()):($=!1,t.innerHTML=Y())}const q="favoriteExercises";function Ce(e){const t=JSON.parse(localStorage.getItem(q))||[];t.push(e._id),localStorage.setItem(q,JSON.stringify(t))}function Me(e){const t=JSON.parse(localStorage.getItem(q))||[],s=t.filter(n=>n!==e._id);localStorage.setItem(q,JSON.stringify(t.push(s)))}function Fe(e){console.log("exerciseData",e),$=!$;const t=document.querySelector(".modal-exercises__btn-favorites"),s=document.querySelector(".favorites__list");$?(t.innerHTML=ee(),s==null?Ce(e):setTimeout(()=>{W()},100)):(t.innerHTML=Y(),s==null?Me(e):setTimeout(()=>{W()},100))}function Y(){return`
  Add to favorites
    <svg class="btn-favorites__icon">
    <use href="${m}#icon-favorites"></use>
  ...
  <svg class="btn-favorites__icon">
    <use href="${m}#icon-trash"></use>
      ...
      </li>`).join("");w.insertAdjacentHTML("beforeend",t)}document.addEventListener("DOMContentLoaded",()=>{const e=document.querySelectorAll(".exercises__btn");e.forEach(t=>{t.addEventListener("click",()=>{e.forEach(s=>s.classList.remove("active")),t.classList.add("active"),ne.classList.add("hidden"),J.classList.add("hidden"),ae.classList.add("hidden")})})});async function v(e,t,s){l=s;try{const{results:n,totalPages:a}=await se.getExercises(e,t,l);re({filter:e,name:t,totalPages:a}),z(n),ie(n)}catch(n){console.log(n)}oe(),Re()}function re({filter:e,name:t,totalPages:s}){if(u.innerHTML="",!(s<=1)){if(s>5){["first","prev"].forEach(i=>{const r=document.createElement("button");l!==1&&(r.classList.add("active-btn"),r.addEventListener("click",()=>{i==="first"?v(e,t,1):i==="prev"&&v(e,t,l-1)})),r.insertAdjacentHTML("beforeend",`<svg class="pagination-icon" width="24" height="24">
                  <use href="${Z}#icon-page-${i}"></use>
                </svg>`),u.appendChild(r)});const a=document.createElement("button");if(a.className="pagination-button-dots",a.insertAdjacentHTML("beforeend","<p>...</p>"),l===1){for(let i=1;i<=3;i++){const r=document.createElement("button");r.className="pagination-button",r.textContent=i,u.appendChild(r),r.addEventListener("click",()=>{v(e,t,i)})}u.appendChild(a)}else if(l===s){u.appendChild(a);for(let i=s-2;i<=s;i++){const r=document.createElement("button");r.className="pagination-button",r.textContent=i,u.appendChild(r),r.addEventListener("click",()=>{v(e,t,i)})}}else{u.appendChild(a);const i=l>3?l-2:1,r=l<s-2?l+2:s;for(let o=i;o<=r;o++){const d=document.createElement("button");d.className="pagination-button",d.textContent=o,u.appendChild(d),d.addEventListener("click",()=>{v(e,t,o)})}u.appendChild(a.cloneNode(!0))}["next","last"].forEach(i=>{const r=document.createElement("button");l!==s&&(r.classList.add("active-btn"),r.addEventListener("click",()=>{i==="last"?v(e,t,s):i==="next"&&v(e,t,l+1)})),r.insertAdjacentHTML("beforeend",`<svg class="pagination-icon" width="24" height="24">
                  <use href="${Z}#icon-page-${i}"></use>
                </svg>`),u.appendChild(r)})}else for(let n=1;n<=s;n++){const a=document.createElement("button");a.className="pagination-button",a.textContent=n,u.appendChild(a),a.addEventListener("click",()=>{v(e,t,n)})}oe()}}const oe=()=>{document.querySelectorAll(".pagination-button").forEach(e=>{e.classList.remove("active-btn"),Number(e.textContent)===l&&e.classList.add("active-btn")})};function Re(){window.scrollTo({top:830,behavior:"auto"})}document.addEventListener("DOMContentLoaded",()=>{ue(),me()});const Oe=new A,Ve=document.getElementById("send-button"),F=document.getElementById("email");Ve.addEventListener("click",ce);F.addEventListener("keydown",function(e){e.key==="Enter"&&ce()});function ce(){const e=F.value;if(!F.checkValidity()){C.error({title:"Error",message:"Please enter a valid email address."});return}Oe.subscribe(e).then(t=>{t.status===201?(C.success({title:"Success",message:t.data.message}),F.value=""):C.error({title:"Error",message:t.data.message})}).catch(t=>{C.error({title:"Error",message:"Network error. Please try again later."})})}const We=new A,De=document.querySelector(".search__button"),le=document.querySelector(".search__input");De.addEventListener("click",function(e){e.preventDefault(),de()});le.addEventListener("keydown",function(e){e.key==="Enter"&&(e.preventDefault(),de())});async function de(e){const{filter:t,name:s}=JSON.parse(localStorage.getItem("paramSearch")),n=le.value.trim();if(n!=="")try{const a=await We.getSearch(t,s,n);z(a)}catch(a){console.log(a)}}
